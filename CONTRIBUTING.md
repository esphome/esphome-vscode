...
For completion and Hover help the extension uses "schema files" which are in the "server/schema" folder.
The schema files are generated first with a script in the esphome repo in the /scripts folder, then the documentation
is injected on those files by another script in the esphome-docs repo.

Validation is done purely by ESPHome itself, the schema files are not used there. It works more or less as described below,
a bit outdated but still relevant in the communication between the extension and ESPHome.

Since the original implementation of this extension, it was running with the architecture detailed below.
However after version 0.3 the extension now uses the Language Server Protocol (LSP).

> Below is original readme written by @OttoWinter and is intended for contributing to the development of this plugin, of course contributions are welcome.

In the current state, _it is not supposed to be used for anything more than testing_. The reason for open-sourcing this plugin in this state is that the developer (@OttoWinter) does not know VSCode nor typescript at all.
...
Each time the websocket is opened, the _dashboard_ will spawn a new esphome process (`esphome --dashboard -q dummy.yaml vscode`). The client uses the stdin/stdout of the process to communicate message frames. The dashboard has three types of websocket messages for this (encoded with JSON):
...
7. When done, the process emits a `type: result` message on stdout with the result of the validation ([source](https://github.com/esphome/esphome/blob/cleanup-dashboard/esphome/vscode.py#L84)).
