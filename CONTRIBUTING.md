...
Completion and Hover help was added, this uses the [YAML LanguageServer](https://github.com/redhat-developer/yaml-language-server) however validation is still perfomed purely agains ESPHome so this feature from the YLS is
disabled. YLS requires JSon schema files, those files are generated by scripts which consumes information from
ESPHome codebase + ESPHome docs parsing.

Validation is done as described below, a bit outdated but still relevant in the communication between the extension
and ESPHome.
